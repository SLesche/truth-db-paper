...
ggplot(plot_study_data, aes(x = variable, y = n, fill = value)) +
...
plot_procedure_data <- long_procedure_data %>%
  ...
  mutate(
    value = fct_relevel(
      factor(value),
      "no", "none", "yes", "both", "exposure", "test", 
      "100", "67", "66.67", "66", "50", "46.67", "33", "0"
    )
  )

# Plot
ggplot(plot_procedure_data, aes(x = variable, y = n, fill = value)) +
  geom_bar(stat = "identity") +
  geom_text(
    aes(label = paste0(value, " (", n, ")")),
    position = position_stack(vjust = 0.5),

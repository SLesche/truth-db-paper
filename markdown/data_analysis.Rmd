...
long_study_data <- study_overview %>% 
  ...
  select(truth_rating_scale, truth_rating_steps, filler_task_yesno, filler_task_type) %>% 
...
model_summary_dichotomous <- parameters::model_parameters(model_full_re_dichotomous, exponentiate = TRUE)

model_summary_scale <- parameters::model_parameters(model_full_re_scale)
...
```{r}
if (compute_new == TRUE){
  # Retentions-Intervall Länge noch mit aufnehmen?
...
model_summary_sameday <- parameters::model_parameters(model_re_likert_sameday)
model_summary_later <- parameters::model_parameters(model_re_likert_later)
```

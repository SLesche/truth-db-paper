...
study_overview_plot <- ggplot(plot_study_data, aes(x = variable, y = n, fill = value)) +
  ...
  theme_minimal(base_size = 20) +  # larger base font size for all text
  theme(
...
ggsave("markdown/images/study_overview_plot.png", study_overview_plot, width = 16, height = 9, scale = 0.7)
...
procedure_overview_plot <- ggplot(plot_procedure_data, aes(x = variable, y = n, fill = value)) +
  geom_bar(stat = "identity") +
  geom_text(
    aes(label = ifelse(str_detect(value, "%") | value == "exposure" | value == "judgment" | value == "both" | value == "none", paste0(value, "\n(", n, ")"), paste0(value, " (", n, ")"))),
    position = position_stack(vjust = 0.5),
    color = "black",
    size = 5,
    lineheight = 0.8,
...
ggsave("markdown/images/procedure_overview_plot.png", procedure_overview_plot, width = 16, height = 9, scale = 0.8)
```

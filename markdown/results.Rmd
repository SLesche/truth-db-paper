...
No random effect on statement seems to be best, which makes sense when seeing the full model.
-> Almost no between statement variance in the repetition effect.
...
We use the "bobqy" optimizer as some models did not converge initially
Was based on x trials, nested in y subject and z procedures. 
here is the model comparison

The full model is best
-> But still - Almost no between statement variance in the repetition effect.

Then, using the scale data (as we had more trials here) split by repetition time (same-day vs not)
Here, the variance between subject is reduced in later

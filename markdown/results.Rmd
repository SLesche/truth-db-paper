...
(ref:model-comp-dichotomous) Overview of Information Criteria for Multi-Level Models of Dichotomous Truth Judgments

```{r model-comp-dichotomous,  tab.cap = paste("(ref:model-comp-dichotomous)"), out.width="100%"}
papaja::apa_table(model_comp_dichotomous, note = "All models included random intercepts at all levels: subject, statement, and procedure. The first model included only random intercepts and the full model included random slopes at all levels. The other three models excluded random slopes at one of the three levels.")
```

All models that included a random slope of repetition at any level outperformed the baseline model with only random intercepts. Detailed model comparisons, based on information criteria, are presented in Table \@ref(tab:model-comp-dichotomous).

The model incorporating random slopes at all hierarchical levels provided the best overall fit. Compared to the model excluding random slopes of statements, it demonstrated a slightly lower AIC and a marginally higher BIC. This is attributable to the minimal variance observed in the repetition effect at the statement level. As expected, the model indicated a significant fixed effect of repetition ($OR =$ `r model_summary_dichotomous$Coefficient[2] %>% apa_num()`, $95\% \ CI =$ `r paste0("[", model_summary_dichotomous$CI_low[2] %>% apa_num(), ", ", model_summary_dichotomous$CI_high[2] %>% apa_num(), "]")`, $p$ `r model_summary_dichotomous$p[2] %>% apa_p()`). Notably, the standard deviation of the random slope of repetition was highest at the subject level ($\sigma =$ `r model_summary_dichotomous$Coefficient[6] %>% apa_num()`), followed by the procedure level ($\sigma =$ `r model_summary_dichotomous$Coefficient[8] %>% apa_num()`), and the statement level ($\sigma =$ `r model_summary_dichotomous$Coefficient[7] %>% apa_num()`).
...
(ref:model-comp-scale) Overview of Information Criteria for Multi-Level Models of Likert-Scale Truth Judgments
```{r model-comp-scale,  tab.cap = paste("(ref:model-comp-scale)"), out.width="90%"}
papaja::apa_table(model_comp_scale, note = "Models were estimated using full maximum likelihood and the 'bobyqa' optimizer. All models included random intercepts at all levels: subject, statement, and procedure. The first model included only random intercepts and the full model included random slopes at all levels. The other three models excluded random slopes at one of the three levels.")
```

The model including random slopes at all hierarchical levels provided the best overall fit, exhibiting improved AIC and BIC values compared to the model that excluded random effects of statements. As expected, the model again indicated a significant fixed effect of repetition ($b =$ `r model_summary_scale$Coefficient[2] %>% apa_num()`, $95\% \ CI =$ `r paste0("[", model_summary_scale$CI_low[2] %>% apa_num(), ", ", model_summary_scale$CI_high[2] %>% apa_num(), "]")`, $p$ `r model_summary_scale$p[2] %>% apa_p()`). Similarly, the standard deviation of the random slope of repetition was highest at the subject level ($\sigma =$ `r model_summary_scale$Coefficient[6] %>% apa_num()`), followed by the procedure level ($\sigma =$ `r model_summary_scale$Coefficient[8] %>% apa_num()`), and the statement level ($\sigma =$ `r model_summary_scale$Coefficient[7] %>% apa_num()`).
...
The same-day model was based on `r nrow(scale_data %>% filter(repetition_time <= 180)) %>% apa_num()` trials nested within `r length(unique(scale_data %>% filter(repetition_time <= 180) %>% pull(subject))) %>% apa_num()` subjects, `r length(unique(scale_data %>% filter(repetition_time <= 180) %>% pull(statement_id))) %>% apa_num()` statements, and `r length(unique(scale_data %>% filter(repetition_time <= 180) %>% pull(procedure_id))) %>% apa_num()` procedures. The model again indicated a significant fixed effect of repetition ($b =$ `r model_summary_sameday$Coefficient[2] %>% apa_num()`, $95\% \ CI =$ `r paste0("[", model_summary_sameday$CI_low[2] %>% apa_num(), ", ", model_summary_sameday$CI_high[2] %>% apa_num(), "]")`, $p$ `r model_summary_sameday$p[2] %>% apa_p()`). The standard deviation of the random slopes at subject level was $\sigma =$ `r model_summary_sameday$Coefficient[6] %>% apa_num()`.

The later-day model was based on `r nrow(scale_data %>% filter(repetition_time > 180)) %>% apa_num()` trials nested within `r length(unique(scale_data %>% filter(repetition_time > 180) %>% pull(subject))) %>% apa_num()` subjects, `r length(unique(scale_data %>% filter(repetition_time > 180) %>% pull(statement_id))) %>% apa_num()` statements, and `r length(unique(scale_data %>% filter(repetition_time > 180) %>% pull(procedure_id))) %>% apa_num()` procedures. The model indicated a significant fixed effect of repetition ($b =$ `r model_summary_later$Coefficient[2] %>% apa_num()`, $95\% \ CI =$ `r paste0("[", model_summary_later$CI_low[2] %>% apa_num(), ", ", model_summary_later$CI_high[2] %>% apa_num(), "]")`, $p$ `r model_summary_later$p[2] %>% apa_p()`). The standard deviation of the random slopes at subject level was $\sigma =$ `r model_summary_later$Coefficient[6] %>% apa_num()`.


**Nochmal der ganze output, falls du noch etwas anderes haben willst?**
